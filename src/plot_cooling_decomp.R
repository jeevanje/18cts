...
coo_int_lim= c(-0.25,3)   # K/day
Hlim_h2o = c(-0.25,3)     # K/day
Hlim_co2 = c(-0.25,0.75)     # K/day
...
for (case in cases){
	...
	#coo2d_s = -pppf2d   # W/m^2/Pa/m^-1, avoid RFM's 3-point stencil?
	...
	Hlim       = eval(as.name(paste("Hlim_",gas,sep="")))
	...
	for (field in fields[1:5]){
	    ...
	    my.image.plot(1e-2*k_coarse,1e-2*p_s[pvec],fac2d*var[ ,pvec],
			ylim = ylim,
			zlim = coo_lim,
			xlab = klab,
			ylab = ylab,
	...
	for (n in 1:length(fields)){
	    ...
	    points(fac1d*var,1e-2*p_s,type="l",lwd=2,lty=lty,col=col)
	}
